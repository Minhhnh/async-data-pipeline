        ...
        async def wrapper(data: Any, monitor: PipelineMonitor):
            for attempt in range(config.retry_attempts):
                try:
                    ...
                    monitor.log_event(f"Sent data to {func.__name__}")
                    return
...
async def file_destination(data: Any, monitor: PipelineMonitor) -> None:
    """Asynchronous file destination for writing data."""
    async with aiofiles.open('output.txt', mode='a') as f:
